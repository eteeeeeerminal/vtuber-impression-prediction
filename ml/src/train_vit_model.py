...
from utils.config import ViTImgRegressorConfig
from dataset.vtuber_imgs_dataset import VTuberImgDataset
from dataset.utils import split_dataset
from model_modules.vit_pl import train_transforms, test_transforms, ViTRegressor


@hydra.main(version_base=None, config_path="../configs", config_name="image_vit_regressor")
def main(cfg: ViTImgRegressorConfig):
    ...
    model = ViTRegressor(dataset.get_label_dim(), cfg.train)

    # checkpoint のセーブの設定
    checkpoint_callback = ModelCheckpoint(
        cfg.save_dir.joinpath("model"), monitor="valid_loss",
        filename="vit-{epoch:02d}-{valid_loss:.4f}",
